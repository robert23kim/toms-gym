...
## Pushup Challenge (backend+frontend shipped 2026-08-01; ENGINE DEPLOY PENDING)
...
- **⚠️ NOT FULLY SHIPPED.** The toms_gym backend + frontend are deployed, but the **analysis engine is not** — the `pushup` config lives on branch `feat/pushup-config` in a worktree at `/tmp/pushup-engine` (repo `~/code/bowling-app/analysis-engine`). Until it deploys, a pushup upload is analyzed with the **bicep_curl** config (`_normalize_lift_type` sends `pushup`, the deployed engine doesn't know it and falls back). Both use the elbow-angle signal, so rep counts are approximately right but form weights are wrong.
- **Deploying the engine:** do NOT run `deploy.sh` as-is — its `--set-env-vars` would drop the live `NORMALIZE_BEFORE_ANALYZE=true`. Build from the worktree (`gcloud builds submit --tag gcr.io/toms-gym/bowling-service`) and deploy with all three env vars (`GCS_BUCKET_NAME`, `PLANK_POSE_WORKERS=8`, `NORMALIZE_BEFORE_ANALYZE=true`). Rollback target: revision `bowling-service-00046-c76`.
- **Analyzer (pending):** a `pushup` entry in `EXERCISE_CONFIGS` reusing the generic elbow-angle peak-detection rep pipeline — no new analyzer math. `rom_extension_deg` 155 / `rom_contraction_deg` 90, `elbow_drift` weighted 0 (hands planted). **ROM thresholds are untuned guesses** — no real pushup video exists in either repo, and mediapipe isn't installed locally, so rep-count accuracy has NOT been validated on real footage. Only synthetic-signal tests pass.
...
- Suite at ship: backend gate **172 passed**; frontend **357 passed**, tsc clean. Analyzer branch: 188 passed, 4 pre-existing `test_estimator` failures that also fail at base commit `f70f646` (unrelated WIP).
