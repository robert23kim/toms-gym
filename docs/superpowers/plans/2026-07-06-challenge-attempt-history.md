...
- [ ] Add optional param to `get_user_lifts` (WHERE clause on page + COUNT queries); run `PYTHON=venv/bin/python tools/run_ci_tests.sh`.
- [ ] Commit — `feat(challenges): competition filter on lift history endpoint`

### Task 2: AttemptHistory component (TDD)
- [ ] Failing tests (time rows `m:ss` + 🏆 max hold; weight rows kg + grade; hrefs; error copy) → red → implement → green + build.
- [ ] Commit — `feat(challenges): per-athlete attempt history panel`

### Task 3: chip + accordion wiring (TDD on LeaderboardRow chip)
- [ ] Extend `LeaderboardRow.test.tsx` (chip at >1 with handler, absent otherwise, click fires toggle) → implement chip props on `LeaderboardRow` + `YouRow` → wire single-open accordion + `<AttemptHistory>` in `ChallengeDetail` → green.
- [ ] Commit — `feat(challenges): expandable leaderboard rows with attempt history`

### Task 4: gate + deploy + docs
- [ ] Full jest + build + tsc + backend CI; deploy `--skip-iam`; prod verify (expand wonder725: 4:04 🏆 / 3:35 / 2:31); CLAUDE.md; mark plan; merge; push.
